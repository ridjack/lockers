...
import io.github.tehstoneman.lockers.api.lockersUtils;
...
public abstract class ComboRecipe implements IRecipe
{
	...
	public boolean matches( InventoryCrafting crafting, World world )
	{
		for( int x = 0; x <= 3 - width; ++x )
			...
			{
				if( checkMatch( crafting, x, y, false ) )
					return true;
				if( canMirror() && checkMatch( crafting, x, y, true ) )
					return true;
			}
		return false;
	}
	...
	{
		final List< ItemStack > shapelessItems = new ArrayList< >();
		for( int x = 0; x < 3; ++x )
			for( int y = 0; y < 3; ++y )
			{
				...
				if( recipeX >= 0 && recipeY >= 0 && recipeX < width && recipeY < height )
					if( mirror )
						recipeStack = recipe[width - recipeX - 1 + recipeY * width];
					else
						recipeStack = recipe[recipeX + recipeY * width];
				final ItemStack craftingStack = crafting.getStackInRowAndColumn( x, y );
				if( craftingStack != null || recipeStack != null )
				{
					...
					if( recipeStack == null && craftingStack != null )
						...
						if( !lockersUtils.wildcardMatch( recipeStack, craftingStack ) )
							return false;
				}
			}
		if( shapelessItems.size() > 0 && !checkShapelessItems( crafting, shapelessItems ) )
			return false;
		return true;
	}
}
