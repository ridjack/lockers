...
import io.github.tehstoneman.lockers.api.stand.ArmorStandEquipHandler;
import io.github.tehstoneman.lockers.api.stand.lockersArmorStand;
import io.github.tehstoneman.lockers.api.stand.EnumArmorStandRegion;
import io.github.tehstoneman.lockers.api.stand.IArmorStand;
import io.github.tehstoneman.lockers.common.tileentity.TileEntityContainer;
...
public class TileEntityArmorStand extends TileEntityContainer implements IArmorStand
{
	...
	public boolean onBlockActivated( EntityPlayer player, int side, float hitX, float hitY, float hitZ )
	{
		...
		for( final ArmorStandEquipHandler handler : lockersArmorStand.getEquipHandlers( region ) )
		{
	...
	public void read( NBTTagCompound compound )
	{
		...
		if( itemsTag instanceof NBTTagList )
		{
			...
			for( int i = 0; i < items.tagCount(); i++ )
			{
				...
				if( slot < 0 || slot >= EnumArmorStandRegion.values().length )
					continue;
				final EnumArmorStandRegion region = EnumArmorStandRegion.values()[slot];
				final ArmorStandEquipHandler handler = lockersArmorStand.getEquipHandler( region, VanillaArmorStandEquipHandler.ID );
				if( handler != null )
		 ...
		 * ArmorStandEquipHandler handler = lockersArmorStand.getEquipHandler(region, id);
