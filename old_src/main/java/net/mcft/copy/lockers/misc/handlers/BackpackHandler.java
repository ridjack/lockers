package net.mcft.copy.lockers.misc.handlers;
...
import net.mcft.copy.lockers.api.lockersBackpack;
...
public class BackpackHandler {
	...
	public BackpackHandler() {
		...
		lockersBackpack.spawnWithBackpack(EntityZombie.class, 1.0 / 800);
		lockersBackpack.spawnWithBackpack(EntitySkeleton.class, 1.0 / 1200);
		lockersBackpack.spawnWithBackpack(EntityPigZombie.class, 1.0 / 1000);
		lockersBackpack.spawnWithBackpack(EntityEnderman.class, 1.0 / 80);
	}
	...
	public void onEntityInteract(EntityInteractEvent event) {
		...
		if (event.entity.worldObj.isRemote ||
		    ...
		    ((event.target instanceof EntityPlayer) &&
		     !lockers.globalConfig.getBoolean(GlobalConfig.enableBackpackInteraction))) return;
	...
	public void onSpecialSpawn(SpecialSpawn event) {
		...
		for (lockersBackpack.BackpackSpawnEntry entry : lockersBackpack.spawnWithBackpack) {
		...
		if (entity.getClass().equals(EntityEnderman.class)) {
			if ((lockersTiles.enderBackpack != null) &&
		...
		} else if (lockersTiles.backpack != null)
		...
		if (backpack == null) {
			...
			if (backpackData.spawnsWithBackpack) {
				...
				if (entity instanceof EntityFrienderman) {
					backpack = new ItemStack(lockersItems.itemEnderBackpack);
					// Remove drop chance for the backpack.
					((EntityLiving)entity).setEquipmentDropChance(EquipmentSlot.CHEST, 0.0F);
				} else {
					backpack = new ItemStack(lockersItems.itemBackpack, 1, RandomUtils.getInt(120, 240));
	...
	public void onLivingDeath(LivingDeathEvent event) {
		...
		if ((player != null) && keepInventory) {
			...
			if (lockers.globalConfig.getBoolean(GlobalConfig.dropBackpackOnDeath)) {
	...
	public void onPlayerLogin(PlayerLoggedInEvent event) {
		...
		lockers.networkChannel.sendTo(new PacketSyncSetting(lockers.globalConfig), event.player);
	}
